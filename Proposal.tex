...
Learning algorithms' efficiency can be amplified through two approaches. One is to improve the processing time; second is to process parallel so as to obtain predicting results faster. For the first approach, there is a special class of optimization problems called convex optimization problems\cite{opt, opt1}. It can efficiently solve real-world problem in a reasonable amount of time with gradient algorithms. One of the applications is SVM where convex optimization method is heavily used. Parallelizability is an important feature for efficient algorithms. Stochastic parallel gradient descent/ascent algorithm is often used for its capability of processing efficiency. Though it had synchronization problem that may affect its performance, but an lock-free approach successfully tackled it\cite{opt2}.

\subsection{Model Selection Evaluation}
In model selection, cross validation is a basic technique. Generally, the dataset S will be split into a training set (70\%) and a cross validation set (30\%). Then the model will be trained and fitted on the training data to get the parameters. Later, we pick the model that render the smallest error on the hold out cross validation dataset\cite{cv}. In case that the data is scarce, k-fold cross validation and leave-one-out cross validation would be the choices for model selection.\\

Other than cross validation, there are evaluation matrixes that can tell the performance of a model. Mean Squared Error, also called Mean Squared Deviation, measures the average of the squares of errors, which is the distance between the predicted value and real value. Taking square root of MSE will get to Root Mean Square Error (RMSE). RMSE is the square root of the variance for an unbiased estimator. Mean Absolute Error, called L1 error as well, is also a loss function that captures the difference between the forecast and the actual outcomes. Another evaluating matrix used often is Receiver Operator Characteristic curve (ROC curve). It depicts the true positive rate and false positive rate on a unit space, and the curve shows the relative tradeoffs between true positive, which is the benefits and false positive, which is loss. The area under the curve (AUC) is an estimator for the probability that a classifier will rank a randomly chosen positive instance higher than a randomly chosen negative one\cite{roc}. \\

Overfitting is a setback for modeling. Regularization is a process or technique to introduce additional information into the loss function to battle against overfitting. Applying regularization to models can improve the generalization of a model. In applications, regulation takes place either explicitly in the form of the functions or by adding constraints to the minimization function\cite{cv, ridge}. Especially for complicated functions or models, minimization algorithm can penalize more complex functions. Tikhonov regularization, or ridge regression, is an example of this case and commonly used. Also, regulation can be implemented in a way to constrain the hypothesis space, such as Ivanov regularization. There is also Lasso Regression analytical method that can be applied for regulation.

\subsection{Feature Selection/Design}
Feature selection is a special case in model selection. Given a large set of n features it is vital to pick a good subset of features to make prediction. One is wrapper model feature selection, which makes calls to the learning model repeatedly with different subsets of data. It includes forward search and backward search. Filter feature selection is much efficient compared with the wrapper method\cite{cv}. For each feature, first calculate a score between this feature and the predicted value. Then this is the mutual information between feature i and dependent variable. Maximum mutual information estimation thus can be used to obtain a valid score for the revealing information of features\cite{mi, mi1}. Finally based on the ranking of the scores for each feature, use cross validation to choose the number of features to be used in the model.\\ 

Reducing dimensionality can also be achieved by using principal component analysis(PCA) or singular value decomposition( SVD)\cite{pca, svd}. These two methods both serve for dimension reduction while retaining important information of the original full data. 
...
\includegraphics[width=0.7\textwidth]{/Users/mingzhexu/Desktop/maggie_xu_thesis/proposal/ROC_space.png} \\Figure. ROC Space (Wikipedia)\\
...
\includegraphics[width=0.7\textwidth]{/Users/mingzhexu/Desktop/maggie_xu_thesis/proposal/ROC.jpg}\\
...
\bibitem{mi}
Bahl, L. R., et al. "Maximum mutual information estimation of hidden Markov model parameters for speech recognition." proc. icassp. Vol. 86. 1986.
\bibitem{dnn1}
...
\bibitem{opt2}
Feng Niu, Benjamin Recht, Christopher Re, Stephen J. Wright: HOGWILD!: A Lock-Free Approach to Parallelizing Stochastic Gradient Descent. \\ Retrieved from: 
\href{url}{http://www.eecs.berkeley.edu/~brecht/papers/hogwildTR.pdf}
\bibitem{roc2}
...
\bibitem{cv}
Ng. Andrew, CS229 Lecture Note, Regularization and model selection. \\Retrieved from \href{url}{http://cs229.stanford.edu/notes/cs229-notes5.pdf}
\bibitem{nb}
...
\bibitem{pca}
Shlens Jonathon: A Tutorial on Principal Component Analysis. \\Retrieved From: \href{url}{http://arxiv.org/pdf/1404.1100v1.pdf}
\bibitem{alphago}
...
\bibitem{ridge}
Tibshirani Ryan: CMU Statistics Notes, Ridge Regression. Retrieved From \\
\href{url}{http://www.stat.cmu.edu/~ryantibs/datamining/lectures/16-modr1.pdf}
\bibitem{svd}
...
\bibitem{mi1}
Notes retrieved from:\\ \href{url}{https://depts.washington.edu/uwcl/twiki/pub/Main/Probability/}mmic.pdf
\bibitem{adaboost}
